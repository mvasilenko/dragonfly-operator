...
func GenerateDragonflyResources(df *resourcesv1.Dragonfly, defaultDragonflyImage string) ([]client.Object, error) {
	...
	// clientPort defaults to DragonflyPort (6379) so existing StatefulSets
	// generated by earlier operator versions stay byte-identical on upgrade
	// and don't rollout. Only a non-nil spec.Port overrides it.
	clientPort := int32(DragonflyPort)
	if df.Spec.Port != nil {
		clientPort = *df.Spec.Port
	}
	...
	// Only set TargetPort when spec.Port is overridden. With a nil spec.Port
	// the generated Service stays byte-identical to what earlier operator
	// versions produced, so existing Services don't churn on upgrade.
	if df.Spec.Port != nil {
		service.Spec.Ports[0].TargetPort = intstr.FromInt32(clientPort)
	}
