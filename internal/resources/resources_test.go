...
// Default path: no probe volumes or mounts (matches upstream behavior).
func TestProbeVolumesAndMounts_DefaultHasNoProbeVolumes(t *testing.T) {
	...
	for _, v := range sts.Spec.Template.Spec.Volumes {
		assert.NotEqual(t, LivenessProbeVolumeName, v.Name, "default should not have liveness probe volume")
		assert.NotEqual(t, ReadinessProbeVolumeName, v.Name, "default should not have readiness probe volume")
		assert.NotEqual(t, StartupProbeVolumeName, v.Name, "default should not have startup probe volume")
	}
	for _, m := range sts.Spec.Template.Spec.Containers[0].VolumeMounts {
		assert.NotEqual(t, LivenessProbeVolumeName, m.Name, "default should not have liveness probe mount")
		assert.NotEqual(t, ReadinessProbeVolumeName, m.Name, "default should not have readiness probe mount")
		assert.NotEqual(t, StartupProbeVolumeName, m.Name, "default should not have startup probe mount")
	}
}

// Custom-probe path: volumes and mounts are present.
func TestProbeVolumesAndMounts_PresentWhenCustomProbeSet(t *testing.T) {
	...
	assert.Equal(t, "my-liveness", volumeNames[LivenessProbeVolumeName])
	assert.Contains(t, volumeNames, ReadinessProbeVolumeName)
	assert.Contains(t, volumeNames, StartupProbeVolumeName)
...
// Default probes use upstream image healthcheck.
func TestProbes_DefaultUseImageHealthcheck(t *testing.T) {
	...
	assert.Nil(t, c.StartupProbe, "default should not have a startup probe")
}

// Custom-probe path: probes point to mounted scripts.
